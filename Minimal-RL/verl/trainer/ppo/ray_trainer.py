...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, decay_rate=1.0):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # decay_rate = getattr(data.config.algorithm, 'decay_rate', 1.0)
        # adv_estimator=self.config.algorithm.adv_estimator
        # decay_rate=self.config.algorithm.decay_rate
        use_decay_rate = decay_rate
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=data.batch['token_level_rewards'],
            response_mask=data.batch['response_mask'],
            index=data.non_tensor_batch['uid'],
            decay_rate=use_decay_rate)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    ...
    def __init__(self,
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True
                ...
                AdvantageEstimator.RLOO, AdvantageEstimator.REINFORCE_PLUS_PLUS_BASELINE, AdvantageEstimator.RAFT,
                AdvantageEstimator.REINFORCE_REJ,
        ]:
